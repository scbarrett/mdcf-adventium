...
modeled by three separate files. Though this does make explanation easier, all
the interfaces *could* have as easily be combined into one file. The files were
generated by `xDML`, the MDCF's DML to AADL translator.

The app presumes that the physiological parameter will be measured by a pulse
oximeter, hence the file's (and package's) name.

And device name...

PCAShutoffApp_pulseox.aadl
^^^^^^^^^^^^^^^^^^^^^^^^^^
The interface through which the app expects to obtain the patient's peripheral 
capillary oxygen saturation (SpO\ :sub:`2`) is declared by this AADL package.
The app presumes that the physiological parameter will be measured by a pulse
oximeter, hence the file's (and package's) name.
